...
使用 Docker Hub 预构建镜像时，首次复制并填写生产环境配置；以后部署或更新只需一条命令：

```bash
cp .env.docker.example .env
# 填写 APP_KEY、APP_URL、DB_PASSWORD、DB_ROOT_PASSWORD 等生产值后：
docker compose -f compose.deploy.yml up -d --pull always --wait
```

该命令会拉取镜像、等待 MySQL、自动执行迁移，再启动 PHP-FPM 与 Nginx；数据库和上传文件保存在命名卷中。镜像发布与首次配置详见[开发手册 · 容器化部署](docs/DEVELOPMENT.md#容器化部署docker)。
