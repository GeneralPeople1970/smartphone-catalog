...
审计工作分支：`workmode/deployment-audit`；合并/发布分支：`main`
Docker 端到端验证提交：`c988197a4bceab03be5b06b20c8ba4efd3218382`
最终结论：**Conditionally Ready**

## 1. 执行摘要

本次审计实际克隆并运行了项目，而不是仅做静态评审。Laravel 以 `APP_ENV=production`、`APP_DEBUG=false` 在隔离 SQLite 数据库上启动，`/up`、首页、构建资源、公开 API、认证与后台 CRUD 完成 87 项 HTTP 巡检并全部通过。修复后的 SQLite 与真实 MySQL 8 完整 PHPUnit 套件最终均为 **161 tests / 662 assertions / 0 failures**；前端 ESLint、Prettier、11 项 Node 测试、后台与 SPA 生产构建均通过。

确认 9 个项目缺陷：Critical 0、High 4、Medium 4、Low 1；9 个均已最小化修复并补回归测试。修复覆盖 MySQL FULLTEXT 测试隔离、公共 API URL 净化、注册/API 限流键冲突、Docker Nginx 上传图片 404、SPA 空白 404 页面、HTTPS 页面混合内容图片，以及 Docker 首次构建/APP_KEY 流程中的三个连续发布阻塞。

本地沙箱访问 Docker daemon socket 被拒绝，但推送后使用 GitHub hosted runner 实际执行仓库 `docker` job。前三次运行依次暴露并修复缺少 ZIP extractor、PHP 8.5 重复编译内置 OPcache、入口脚本污染 APP_KEY 输出三个阻塞；最终 [CI run 29503142967](https://github.com/GeneralPeople1970/smartphone-catalog/actions/runs/29503142967) 的镜像构建、APP_KEY、MySQL migration、Compose up 和 `/up` smoke test 全部通过。容器重启/镜像重建后的卷持久化、镜像内容/大小和真实浏览器巡检仍未完成，因此结论仍为 **Conditionally Ready**。

主测试矩阵最终执行计数为 **420 passed / 0 failed**（SQLite 161 + MySQL 161 + 前端 11 + HTTP 87），另有 6/6 Nginx 运行时探针和 GitHub Actions 4/4 jobs 符合预期。定向安全套件 83 项是上述 PHPUnit 套件的重复抽取，不计入 420 的唯一执行总数。
...
实际执行覆盖：依赖安装、平台要求、语法、Pint、PHPUnit、SQLite/MySQL 迁移和回滚、配置/路由/视图缓存、npm audit、ESLint、Prettier、Node tests、双 Vite build、Gitleaks、生产模式 HTTP、API/认证/权限/CRUD、安全输入、限流、Nginx 上传与敏感路径探针、Compose 配置解析、本地 Docker daemon 探测，以及 GitHub Actions 上的 Docker build/migrate/up/health smoke。
...
| --- | --- | --- | --- |
| `test` / PHP | validate、install、audit、platform、Pint、PHPUnit | PASS | 161/161，662 assertions |
| `test` / Node | 两次 `npm ci`、两次 audit、check、build | PASS | ESLint 0 warning；Prettier pass；11/11 tests |
| `mysql-test` | MySQL 8 空库迁移 + fulltext PHPUnit | PASS after fix | 基线 153 中 2 项失败；最终 161/161，662 assertions |
| `docker` / config | Compose 2.40.3 解析四服务/两卷/网络 | PASS | 缺仓库 `.env` 的原始 config 失败符合文档；隔离临时环境解析成功 |
| `docker` / build/up | build → key → migrate → up → `/up` | PASS after fixes | GitHub Actions run 29503142967，docker job 1m12s |
| `secret-scan` | Gitleaks 全历史 | PASS | 44 commits，0 leaks |

本地复现后又触发并监控远程 GitHub Actions。最终 run 的 `test`、`mysql-test`、`docker`、`secret-scan` 四个 jobs 全部成功。所有 Actions 在仓库中均固定完整 SHA，顶层权限为 `contents: read`。
...
- 最终完整 PHPUnit：**161 tests、662 assertions、0 failures**（新增 3 项 Docker 回归测试）。
...
- 最终 GitHub `mysql-test`：**161 tests、662 assertions、0 failures**；应用/数据库断言与本地 158 项结果一致，另包含 3 项 Docker 回归测试。
...
本地 `docker compose build` 到达 daemon 探测后被沙箱拒绝：
...
为完成真实部署验证，将修复提交推送到 `main` 后由 GitHub hosted runner 运行仓库原始 CI docker job。连续失败证据与修复如下：

| Run / commit | 到达阶段 | 失败证据 | 修复 |
| --- | --- | --- | --- |
| `29502359025` / `1c96431` | Composer vendor | `zip extension and unzip/7z commands are both missing` | vendor build stage 安装 `unzip` |
| `29502723920` / `9768f17` | PHP runtime extensions | PHP 8.5 OPcache 已内置，重复安装导致 `cp: cannot stat 'modules/*'` | 移除 `docker-php-ext-install opcache`，保留 module assertion |
| `29502929233` / `3488882` | Generate APP_KEY | entrypoint 多行输出导致 `sed: unterminated 's' command` | 仅在 `php-fpm` 启动时执行 link/cache hooks |
| `29503142967` / `c988197` | 完整 docker job | 无失败 | **PASS**：build、key、migrate、up、`/up`、teardown |

最终 CI 实证：runtime/web 镜像成功构建；随机 CI APP_KEY 成功写入；一次性 migrate 等待健康 MySQL 并成功；`db/app/web` 启动；Nginx → PHP-FPM → MySQL 的 `/up` 返回 200；任务结束后只清理该 CI 创建的 volumes。
...
- runtime/web 镜像大小和 layer 膨胀检查

其余可执行验证：Dockerfile/Compose/`.dockerignore` 静态边界检查；Laravel production-mode 部署；真实 MySQL；真实 Nginx location 解析/请求；上传 alias 与脚本阻止探针。Nginx 修复后合法上传图片 200、PHP 403、隐藏文件 403。

证据：[`docker-compose.log`](artifacts/docker-compose.log)、[`docker-build.log`](artifacts/docker-build.log)、[`docker-environment.log`](artifacts/docker-environment.log)、[`nginx-storage-initial-failure.log`](artifacts/nginx-storage-initial-failure.log)、[`nginx-storage-regression.log`](artifacts/nginx-storage-regression.log)、[最终 GitHub CI](https://github.com/GeneralPeople1970/smartphone-catalog/actions/runs/29503142967)。
...
- `GET /up` → 200
- GitHub Compose `/up` 经 Nginx → PHP-FPM → MySQL → 200
...
| WM-007 | High | Docker build | production vendor stage 无法安装 Composer dist 包，镜像构建中止 | CI run 29502359025：`docker compose build` | `php:8.5-cli` 无 ext-zip，也无 `unzip/7z` | 只在 build-only vendor stage 安装并清理 `unzip` | `DockerfileTest` + CI build 越过 vendor stage | Fixed |
| WM-008 | High | Docker/PHP 8.5 | runtime 扩展安装在 `modules/*` 阶段失败 | CI run 29502723920：runtime image build | PHP 8.5 已将 OPcache 内置，Dockerfile 仍尝试作为共享扩展编译安装 | 从安装列表移除 OPcache，并断言内置 module 存在 | `DockerfileTest` + CI image build PASS | Fixed |
| WM-009 | High | Docker release | APP_KEY 生成输出无法写入 `.env`，首次部署中止 | CI run 29502929233：Generate APP_KEY | app entrypoint 对一次性 artisan 命令也执行并输出启动 hooks，污染命令替换 | 仅 `php-fpm` 启动执行 link/cache，一次性命令直接 exec | `DockerEntrypointTest` + CI key/migrate/up PASS | Fixed |

严重程度统计：Critical 0、High 4、Medium 4、Low 1。确认缺陷 9，已修复 9，未修复确认缺陷 0。
...
| `Dockerfile` | vendor stage 补 ZIP extractor；PHP 8.5 使用内置 OPcache |
| `docker/entrypoint.sh` | 一次性命令跳过 php-fpm 启动 hooks |
...
| `tests/Unit/DockerfileTest.php` | vendor extractor 与 PHP 8.5 OPcache 构建守护 |
| `tests/Unit/DockerEntrypointTest.php` | 一次性容器命令输出纯净性回归 |
| `frontend/tests/image-url-safety.test.mjs` | Mixed Content 前端回归 |
| `frontend/tests/router.test.mjs` | 404 路由回归 |

没有主要依赖升级、无无关重构、无降低断言、无吞异常、无 `|| true` 掩盖项目命令。修复已合并并推送到 `main`；关键提交为 `1c96431`、`9768f17`、`3488882`、`c988197`。
...
- 新增：Docker vendor stage 在 Composer install 前提供 ZIP extractor。
- 新增：PHP 8.5 runtime 不重复编译内置 OPcache，并验证 module 存在。
- 新增：Docker 一次性命令不输出 php-fpm 启动 hooks。
...
- `git diff --check`：通过。
- GitHub CI run 29503142967：成功；Docker build/key/migrate/up/`/up` 全链路通过。
...
| 实际容器 UID/镜像内容/大小 | build/up 已通过，但未做深度镜像审计 | Environment Blocked | `docker inspect/exec/history/image save` |
...
最后两项是仓库已明确说明的权衡/风险，不计入本次 9 个代码缺陷。没有未解决的 Critical 或 High 代码缺陷。
...
| 空库全量 migration | 是 |
| Docker 镜像构建 | 是，GitHub hosted runner 实证 |
| Docker Compose 正常启动 | 是，db/app/web + one-shot migrate |
| `/up` | 是，本地及 Docker Nginx 链路均 200 |
| 首页和关键 API | 是，87/87 HTTP |
| 未解决 Critical/High | 是，无未解决项 |
| production debug off | 是，本地实测 + 模板检查 |
| 敏感文件不可通过 Web 读取 | 本地内容检查 + Nginx 403 探针通过；镜像内容未做 image-save 深审计 |
...
代码、完整测试和 GitHub Docker build/up 均通过，且没有未解决 Critical/High 缺陷；但容器重启/重建后的数据库与上传持久化、镜像深度审计和真实浏览器巡检仍是用户定义的硬性完成标准，当前环境无法证明，故不能标记 Ready。全部修复已推送到 `main`。

## 21. 建议的后续行动

1. 使用唯一 Compose project name 和随机临时凭据，创建专用 db/upload volumes；写入标记数据和上传文件，依次执行 restart、app/web rebuild，再确认数据仍在。只删除该测试 project 的 volume。
2. 执行镜像审计：大小、layer、`.env`/test DB/node_modules/tests/docs、容器用户、PHP-FPM/Nginx worker、上传目录脚本执行；记录 `docker inspect/history`。
3. 把 `artifacts/nginx-storage-regression.log` 的图片 200 / script 403 探针加入 Docker CI，补足静态配置测试之外的端到端守护。
4. 在可访问 staging URL 的 Playwright/浏览器上跑桌面与移动视口：首页轮播、主题持久化、前进后退、404、错误/空状态、图片回退、长中英文本、键盘与基本无障碍、Console/Network。
5. 评估 nonce CSP 与 Alpine CSP build，移除 `'unsafe-inline'/'unsafe-eval'`；在 TLS 终止层验证 HSTS 与 trusted proxy。
