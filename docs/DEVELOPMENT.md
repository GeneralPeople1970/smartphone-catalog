...
- **阶段 1（`node:22-alpine`）**：`npm ci` + `npm run build`，产出 `public/build`（后台）与 `public/frontend`（前台）。
- **阶段 2（`php:8.4-cli` + composer）**：`composer install --no-dev` + `dump-autoload --optimize --classmap-authoritative`。
- **阶段 3 `runtime`（`php:8.4-fpm`）**：安装运行期扩展 `pdo_mysql`、`gd`、`zip`、`bcmath`、`opcache`（`fileinfo` 官方镜像自带并校验存在），启用 OPcache（`docker/php/opcache.ini`，`validate_timestamps=0`，改代码需重建镜像）与上传限制（`docker/php/php.ini`，`upload_max_filesize`/`post_max_size` 24M）。`package:discover` **不**再 `|| true`，发现失败即构建失败。以 `www-data` 非 root 运行，`ENTRYPOINT` 每容器缓存 config/route/view 后起 `php-fpm`。
